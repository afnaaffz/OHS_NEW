...
def update(request,id):
    ...
    if request.method == 'POST':
        form = Register_Form1(request.POST,request.FILES,instance=a)
        if form.is_valid():
            form.save()
            return redirect("workers_data")
...
@login_required(login_url = 'login_page')
def worker_view_filtered_schedule(request):
    u = Register1.objects.get(user=request.user)
    data = Schedule.objects.filter(worker=u)
    return render(request,"worker/worker_view_filtered_schedule.html",{"data": data})
...
def view_profile(request):
    data = Register1.objects.filter(user=request.user)
    print(data)
...
def edit_profile(request,id):
    ...
    return render(request, "worker/edit_profile.html", {'form': form})
